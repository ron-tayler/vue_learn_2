    ...
    <div class="profile-body tab-timeline">
        <div class="card-block">
            ...
            <div class="timeline-btn-box">
                ...
                <div class="btn btn-light" @click="backWeek"><fa-icon icon="chevron-left"></fa-icon></div>
                <div class="btn btn-light" @click="nextWeek"><fa-icon icon="chevron-right"></fa-icon></div>
                <div class="dropdown-box" style="display: inline-block">
                    <div class="btn btn-light" @click="isDDM=!isDDM">
                        <fa-icon icon="calendar-alt"></fa-icon>
                    </div>
                    <div class="dropdown-menu dropdown-right timeline-calendar-box"
                         v-if="isDDM">
            ...
            <div class="timeline-grid">
                ...
                <div class="timeline-hours-line">
                    <div class="hours-box" v-for="(hour,i) in timelineHours" :key="i" :class="{active:hour.date===hour_active}">
                        <span class="hours">{{hour.name}}</span>
                    </div>
                </div>
                <template v-for="(day,i) in timelineDays">
                    <div class="timeline-day-info" :key="i*2" :class="{active:day.str===day_active}">
                        <span>{{day.name}}</span>
                        <span>{{day.date}}</span>
                    </div>
                    <div class="timeline-day-line" :key="i*2+1" :class="{active:day.str===day_active}">
                        <transition-group :name="transName">
                        <div class="timeline-card"
                             v-for="card in week_cards[i]" :key="card.UTC_timestamp_start"
                             :style="getStylePos4time(card)">
                            <span class="timeline-card-name">{{card.name}}</span>
                            <span>
                                <span class="timeline-card-category">{{card.category}} </span>•
                                <span class="timeline-card-view"> {{card.views}} просмотров</span>
                            </span>
...
<script>
    ...
    export default {
        ...
        MONTH: {
            ...
            'ru':['янв.','фев.','мар.','арп.','май.','июн.','июл.','авг.','сен.','окт.','ноя.','дек.']
        },
        props:['user_id'],
        ...
        computed:{
            between_text(){
                ...
                return week_start.getDate()+' '+
                    this.$options.MONTH[this.$store.getters.getLang][week_start.getMonth()]+' '+
                    week_start.getFullYear()+'г. - '+
                    week_finish.getDate()+' '+
                    this.$options.MONTH[this.$store.getters.getLang][week_finish.getMonth()]+' '+
                    week_finish.getFullYear()+'г.'
            },
            timelineHours(){
            ...
            timelineDays(){
                ...
                for(let i=0;i<7;i++){
                    ...
                    arr[i] = {
                        ...
                        date:(day.getDate()<10?'0':'')+day.getDate()+'.'+(day.getMonth()+1<10?'0':'')+(day.getMonth()+1),
        ...
        methods:{
            getStylePos4time(card){
                ...
                let dateFinish = new Date(card.UTC_timestamp_start + card.length_seconds*1000);
            ...
            getTimezone4offset(offset) {
                ...
                return (offset===0)?'GMT':(offset>0?'GMT+':'GMT-')+gmt_h+(gmt_m===0?'':':'+(gmt_m<10?'0'+gmt_m:gmt_m));
            },
            setActiveHour8Day(){
                let now = new Date();
            ...
            setWeek(date){
                this.week_start = new Date(date.valueOf() - (date.getDay()-1)*24*60*60*1000).valueOf();
            },
            setNowWeek(){
                let now = new Date();
        ...
        watch:{
            week_start(week_start,old){
                ...
                let filteredStreams = streams.filter(
                    stream => {
                        return stream.UTC_timestamp_start > week_start &&
                        stream.UTC_timestamp_start < week_finish
                    }
                );
                for(let weekDay = 0;weekDay<7;weekDay++){
                    this.week_cards[weekDay] = filteredStreams.filter(
                        stream => {
                            return stream.UTC_timestamp_start > week_start + weekDay * 24 * 60 * 60 * 1000 &&
                            stream.UTC_timestamp_start < week_start + (weekDay + 1) * 24 * 60 * 60 * 1000
                        }
                    );
                }
            }
